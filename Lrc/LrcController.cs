...
namespace Lyrics
{
    public class LrcController
    {
        ...
        public static string defaultDir = (Directory.GetCurrentDirectory() + @"\lyric\");
        private static Font defaultFont = new Font("微软雅黑", 24f, FontStyle.Bold, GraphicsUnit.Pixel);
        private static string defaultText = "Love Music，Love Music！";
        ...
        public static void ChangeColor(int index)
        {
            switch (index)
            {
                ...
                case 2:
                    bgUPColor = System.Drawing.Color.LightPink;
                    bgDownColor = System.Drawing.Color.Pink;
                    break;

                case 3:
                    bgUPColor = System.Drawing.Color.Yellow;
                    bgDownColor = System.Drawing.Color.DarkOrange;
                    break;
        ...
        public static void searchLrc(string fileName)
        {
            lyric._Clear();
            lrcWindow.Init(fileName);
            foreach (string str in LrcTypes)
            {
                FileInfo[] files = new DirectoryInfo(defaultDir).GetFiles(str, SearchOption.AllDirectories);
                if (files != null)
                {
        ...
        public static void setVolue(int n)
        {
            if (n == 0)
            {
                lrcToolBar.volume.ToolTip = "打开声音";
               // lrcToolBar.volume.Source = getImageSource(System.Resources.noVolume);
            }
            else
            {
                lrcToolBar.volume.ToolTip = "静音";
               // lrcToolBar.volume.Source = getImageSource(System.Resources.volume);
            }
        }
